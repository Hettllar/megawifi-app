            ...
            <div class="bg-yellow-500/20 border border-yellow-400/30 rounded-xl p-3 sm:p-4 mb-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs sm:text-sm text-purple-200">نفذ السكريبت وسيتصل تلقائياً</p>
                    </div>
                </div>
            </div>
        @else
...
/interface wireguard add name=wg-megawifi listen-port=13231{{ $router->wg_private_key ? ' private-key="' . $router->wg_private_key . '"' : '' }} comment="MegaWiFi VPN"
/ip address add address={{ $router->wg_client_ip }}/24 interface=wg-megawifi comment="MegaWiFi WG"
/interface wireguard peers add interface=wg-megawifi public-key="{{ config('wireguard.server_public_key') }}" endpoint-address={{ explode(':', config('wireguard.endpoint'))[0] }} endpoint-port={{ explode(':', config('wireguard.endpoint'))[1] ?? '51820' }} allowed-address=10.10.0.0/24 persistent-keepalive=25 comment="MegaWiFi Server"
/ip firewall filter add chain=input protocol=udp dst-port=13231 action=accept comment="WireGuard MegaWiFi" place-before=0
/ip firewall filter add chain=input src-address=10.10.0.0/24 action=accept comment="MegaWiFi Network" place-before=0
...
:put $myKey</pre>
        ...
        <div class="mt-4 bg-white/10 rounded-xl p-3 sm:p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                ...
                <button onclick="toggleEditKey()" id="editKeyToggle" class="bg-yellow-500/30 hover:bg-yellow-500/50 border border-yellow-400/50 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold">
                    <i class="fas fa-edit ml-1"></i>تعديل المفتاح
                </button>
            </div>
            <div class="bg-black/30 rounded-lg p-2 sm:p-3 font-mono text-xs sm:text-sm break-all">
                <span class="text-green-300">{{ $router->wg_public_key }}</span>
            </div>
            <!-- Edit form hidden by default -->
            <div id="editKeySection" class="hidden mt-3 bg-yellow-500/10 border border-yellow-400/30 rounded-xl p-3 sm:p-4">
                <p class="text-yellow-200 text-xs sm:text-sm mb-2"><i class="fas fa-exclamation-triangle ml-1"></i>تحذير: تغيير المفتاح سيقطع الاتصال الحالي حتى يتم تنفيذ السكريبت مجدداً على الراوتر</p>
                <form action="{{ route('routers.wireguard.save-public-key', $router) }}" method="POST" id="editPubKeyForm">
                    @csrf
                    <div class="flex flex-col sm:flex-row gap-2">
                        <input type="text" name="public_key" placeholder="المفتاح العام الجديد - 44 حرف" required
                        ...
                        <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 px-4 sm:px-6 py-2 rounded-lg font-bold text-sm text-black">
                            <i class="fas fa-save ml-1"></i>تحديث المفتاح
                        </button>
                    </div>
                </form>
            </div>
        </div>
        @elseif(!$router->wg_public_key || $router->wg_public_key === config('wireguard.server_public_key'))
        <div class="mt-4 bg-white/10 rounded-xl p-3 sm:p-4">
    ...
    <!-- Quick Stats -->
        ...
        <!-- ZeroTier VPN Section -->
        @if($router->zt_enabled)
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
            <h3 class="text-lg font-bold text-blue-800 flex items-center gap-2 mb-3">
                <i class="fas fa-network-wired"></i>
                اتصال ZeroTier VPN
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-gray-500">Network ID</p>
                    <p class="font-mono font-bold text-blue-900">{{ $router->zt_network_id }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Member ID</p>
                    <p class="font-mono font-bold text-blue-900">{{ $router->zt_member_id ?? 'غير محدد' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">ZeroTier IP</p>
                    <p class="font-mono font-bold text-blue-900">{{ $router->zt_ip }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">الحالة</p>
                    @if($router->zt_connected)
                        <p class="text-green-600 font-bold"><i class="fas fa-check-circle"></i> متصل</p>
                    @else
                        <p class="text-red-600 font-bold"><i class="fas fa-times-circle"></i> غير متصل</p>
                    @endif
                </div>
                <div>
                    <p class="text-sm text-gray-500">آخر اتصال</p>
                    <p class="text-gray-700">{{ $router->zt_last_seen ? $router->zt_last_seen->diffForHumans() : 'لم يتصل بعد' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">طريقة الاتصال الفعالة</p>
                    <p class="font-bold text-blue-700">
                        @if($router->connection_method === 'zerotier')
                            <i class="fas fa-network-wired"></i> ZeroTier
                        @elseif($router->connection_method === 'wireguard')
            ...
            <div class="mt-3 pt-3 border-t border-blue-200">
                <p class="text-sm text-blue-600">
                    ...
                    IP الفعال المستخدم: <span class="font-mono font-bold">{{ $router->effective_ip }}</span>
                </p>
            </div>
        </div>
        @endif
...
function toggleEditKey() {
    const section = document.getElementById('editKeySection');
    const btn = document.getElementById('editKeyToggle');
    if (section.classList.contains('hidden')) {
        section.classList.remove('hidden');
        btn.innerHTML = '<i class="fas fa-times ml-1"></i>إلغاء';
        btn.className = 'bg-red-500/30 hover:bg-red-500/50 border border-red-400/50 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold';
    } else {
        section.classList.add('hidden');
        btn.innerHTML = '<i class="fas fa-edit ml-1"></i>تعديل المفتاح';
        btn.className = 'bg-yellow-500/30 hover:bg-yellow-500/50 border border-yellow-400/50 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold';
    }
}

// Handle edit public key form
document.addEventListener('DOMContentLoaded', function() {
    const editForm = document.getElementById('editPubKeyForm');
    if (editForm) {
