...
class WireGuardService
{
    ...
    public function getNextAvailableIP(): string
    {
        ...
        $usedIPs = Router::whereNotNull('wg_client_ip')
            ...
            ->map(function ($ip) {
                // Extract last octet
                preg_match('/10\.10\.0\.(\d+)/', $ip, $matches);
        ...
        preg_match_all('/10\.10\.0\.(\d+)/', $wgOutput, $wgMatches);
    ...
    public function addPeerToServer(Router $router): bool
    {
        ...
        $cmd = sprintf(
            'sudo /usr/bin/wg set %s peer %s allowed-ips %s/32 persistent-keepalive 25 2>&1',
            $this->wgInterface,
    ...
    public function generateRouterOSScript(Router $router): string
    {
        ...
        $privateKeyParam = $router->wg_private_key ? " private-key=\"{$router->wg_private_key}\"" : "";
        $script = <<<ROUTEROS
# ═══════════════════════════════════════════════════════════════════
...
# Step 2: Create WireGuard Interface with platform keys
/interface wireguard add name={$interfaceName} listen-port=13231{$privateKeyParam} comment="MegaWiFi Management Tunnel"
...
/interface wireguard peers add interface={$interfaceName} public-key="{$serverPublicKey}" endpoint-address={$serverIP} endpoint-port={$serverPort} allowed-address=10.10.0.0/24 persistent-keepalive=25 comment="{$peerName}"
...
/ip firewall filter add chain=input src-address=10.10.0.0/24 action=accept comment="Allow MegaWiFi Management" place-before=0

# Step 6: Enable API for MegaWiFi network
/ip service set api address=10.10.0.0/24 disabled=no
/ip service set api-ssl address=10.10.0.0/24 disabled=no
...
ROUTEROS;
    ...
    public function generateOneLinerScript(Router $router): string
    {
        ...
        $privateKeyPart = $router->wg_private_key ? ' private-key="' . $router->wg_private_key . '"' : '';
        $script = "/interface wireguard add name=wg-megawifi listen-port=13231{$privateKeyPart}; ";
        $script .= "/ip address add address={$router->wg_client_ip}/24 interface=wg-megawifi; ";
        $script .= "/interface wireguard peers add interface=wg-megawifi public-key=\"{$serverPublicKey}\" endpoint-address={$serverIP} endpoint-port={$serverPort} allowed-address=10.10.0.0/24 persistent-keepalive=25; ";
        $script .= "/ip firewall filter add chain=input protocol=udp dst-port=13231 action=accept place-before=0; ";
        $script .= "/ip firewall filter add chain=input src-address=10.10.0.0/24 action=accept place-before=1; ";
        $script .= "/ip service set api address=10.10.0.0/24 disabled=no; ";
